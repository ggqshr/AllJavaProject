			...
			<form id="form1" action="test1" method="post">
				<table class="table table-border table-bordered table-bg">
					...
					<tbody>
						...
						<tr>
							<%
        List<Waiter> foods=(List<Waiter>)session.getAttribute("list");
        String adr=(String)session.getAttribute("adr");
        int num=1;
        for (Waiter food : foods) {
			
         %>
						
						<tr>
							<th><%=num++ %></th>
							<th><%=food.getWaiterNumber() %></th>
							<th><%=food.getWaiterName() %></th>
							<th><%=food.getWaiterMoney() %></th>
						</tr>
						<% 
		}
		 %>
