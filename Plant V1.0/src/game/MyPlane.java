...
//			return false;
	...
	public boolean hitBossBullet(BossBullet bossBullet)
	{
		if (bossBullet.getX()>x+10 && bossBullet.getX()<(x + width-10)) {
			if (bossBullet.getY()>y+10&&bossBullet.getY()<y+height-10) {
				return true;
			}
			return false;
		}
	...
	{
		if(x>boss.getX()+boss.getWidth()||x+width<boss.getX())
		{
			return false;
		}
		if(y>boss.getY()+height||y+height<boss.getY())
		{
			return false;
		}
		return true;
