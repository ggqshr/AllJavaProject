...
public class Enemy {//小飞机
	...
	//构造方法  初始化属性
	public Enemy (int speed,int y) {
		x = random.nextInt(480);//初始化横坐标
		this.y = y ;//初始化纵坐标
		this.speed = speed;
		try {
			image = ImageIO.read(getClass().getResource("enemy0.png"));//初始化图片
		} catch (IOException e) {
		...
		for (int i = 0; i < images.length; i++) {
			try {
				images[i] = ImageIO.read(getClass().getResource("enemy0_down"+(i+1)+".png"));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
